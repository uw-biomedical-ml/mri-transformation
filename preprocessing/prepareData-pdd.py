...
destination_dir = "/data/mri/data/pdd_sliced"
if not os.path.exists(destination_dir):
  os.makedirs(destination_dir)

A_dir = "/data/mri/data/raw-with-dti/resliced_to_dti"
B_dir = "/data/mri/data/raw-with-dti/pdd"
...
def split_data(new_split = True):
  if new_split:
    ...
    B_files = glob.glob("{}/*-pdd.nii.gz".format(B_dir))
    N = len(B_files)
...
def get_slices(split, B_files):
  ...
  def get_A_file(B_file, A_key):
    A_file = os.path.basename(B_file).replace('pdd.nii.gz', A_key)
  ...
  for B_file in B_files:
    ...
    for i in range(T1_data.shape[2]):
      ...
      B_slice_np = (B_data[:,:,i,:] + 1) / 2 ## this is supposed to change the value from [-1,1] to [0,1]
      AB_np = np.concatenate([A_slice_np, B_slice_np], 1)
      np.save("%s/%s_%04d" % (AB_to_dir, subject_id, i), AB_np)

train_files, val_files, test_files = split_data(new_split=True)
